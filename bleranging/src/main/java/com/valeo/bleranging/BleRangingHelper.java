...
public class BleRangingHelper {
    ...
    private final Runnable sendPacketRunner = new Runnable() {
        ...
        public void run() {
            ...
            mBluetoothManager.sendPackets(bytesToSend, bytesReceived,
                    mProtocolManager.getPacketTwoPayload(connectedCar.getStandardClasses(),
                            connectedCar.getStandardDistribution()),
                    mProtocolManager.getPacketThreePayload(connectedCar.getStandardRssi()),
                    mProtocolManager.getPacketFourPayload(connectedCar.getPredictionCoord(), connectedCar.getDist2Car()));
        ...
        public void run() {
            if (isLoggable) {
                LogFileUtils.appendRssiLogs(
                        ...
                        mAlgoManager.getOrientation()[0],
                        mAlgoManager.getOrientation()[1],
                        mAlgoManager.getOrientation()[2],
                        mAlgoManager.getGravity()[0],
                        mAlgoManager.getGravity()[1],
                        mAlgoManager.getGravity()[2],
                        mAlgoManager.getGeomagnetic()[0],
                        mAlgoManager.getGeomagnetic()[1],
                        mAlgoManager.getGeomagnetic()[2],
                        mAlgoManager.getAcceleration(),
                        ...
                        beepInt,
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_LEFT),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_MIDDLE),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_RIGHT),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_TRUNK),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_FRONT_LEFT),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_FRONT_RIGHT),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_REAR_LEFT),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_REAR_RIGHT),
                        connectedCar.getCurrentAntennaId(NUMBER_TRX_BACK));
                beepInt = 0;
            }
            if (isFullyConnected()) {
                mMainHandler.postDelayed(this, 105);
            ...
            if (BluetoothLeService.ACTION_DATA_AVAILABLE.equals(action)) {
                ...
                if (bytesReceived != null) {
                    ...
                    if (bytesReceived.length >= 7) {
                        ...
                        if (infoType == 0x00) {
                            final byte[] address = Arrays.copyOfRange(bytesReceived, 7, 12);
                            connectedCar.saveCarAddress(receivedTrxNumber,
                                    TextUtils.printAddressBytes(address));
                        } else if (infoType == 0x01) {
                            connectedCar.saveCarRssi(receivedTrxNumber, bytesReceived[12]);
                        }
                    }
                    lock.readLock().unlock();
                }
    ...
    private void updateCarLocalization(String predictionPosition,
                                       String predictionProximity, PointF coord, double dist) {
        ...
        chessBoardListener.updateChessboard(coord, dist);
    }
    ...
    public String[] getStandardClasses() {
        if (connectedCar != null) {
            return connectedCar.getStandardClasses();
        }
        return null;
    }
